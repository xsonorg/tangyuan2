...
public abstract class AbstractHiveNode extends AbstractServiceNode {
	...
	public boolean execute(ActuatorContext ac, Object arg, Object acArg) throws Throwable {
		...
		if (null != this.cacheUse) {
			...
			if (null != result) {
				...
				if (getLog().isInfoEnabled()) {
					getLog().info("hive execution time: " + getSlowServiceLog(startTime) + " use cache");
				}
				return true;
			}
		}
		...
		result = executeSql(ac, sqlContext, acArg);

		if (getLog().isInfoEnabled()) {
			getLog().info("hive execution time: " + getSlowServiceLog(startTime));
		}
