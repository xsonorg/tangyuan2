...
public class HiveTextNode implements TangYuanNode {
	...
	public HiveTextNode(String text) {
	...
	public boolean execute(ActuatorContext ac, Object arg, Object acArg) throws Throwable {
		...
		if (null == this.dynamicVarList) {
			...
			if (null == this.staticVarList) {
				serviceContext.addSql(this.staticSql);
			} else {
				serviceContext.addSql(this.staticSql);
				serviceContext.addStaticVarList(this.staticVarList, acArg);
			}
		} else {
			// 每次解析
			...
			for (Object obj : this.dynamicVarList) {
				if (obj instanceof ShardingVariable) {
					ShardingResult result = (ShardingResult) ((ShardingVariable) obj).getValue(acArg);
					// 设置数据源
					serviceContext.setDsKey(result.getDataSource());
					// 设置表名, 有可能为空字符串*
					builder.append(result.getTable());
				} else if (obj instanceof Variable) {
					builder.append(((Variable) obj).getValue(acArg));
				} else {
			...
			if (null == this.staticVarList) {
				serviceContext.addSql(parsedText);
			} else {
				serviceContext.addSql(parsedText);
				serviceContext.addStaticVarList(this.staticVarList, acArg);
			}
