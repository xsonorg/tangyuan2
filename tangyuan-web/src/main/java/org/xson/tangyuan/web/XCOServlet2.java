...
public class XCOServlet2 extends HttpServlet {

	private static final long serialVersionUID       = 1L;

	private static Log        log                    = LogFactory.getLog(XCOServlet2.class);
	...
	private void handler(HttpServletRequest req, HttpServletResponse resp, RequestTypeEnum requestType) {
		...
		try {
			if (!container.isRunning()) {
				log.error(TangYuanLang.get("web.servlet.invalid.state", requestURI));
			...
			if (!container.isRestMode()) {
				if (RequestTypeEnum.POST != requestType && RequestTypeEnum.GET != requestType) {
					log.error(TangYuanLang.get("web.servlet.invalid.requestType", requestURI, requestType));
					throw new ServiceException(container.getErrorCode(), container.getErrorMessage());
				}
			}
		} catch (ServiceException se) {
			doResponseError(req, resp, se, null, null);
			return;
		}
		...
		if (null == cVo) {
			log.error(TangYuanLang.get("web.servlet.invalid.controller", requestURI));
			ServiceException se = new ServiceException(container.getErrorCode(), container.getErrorMessage());
			doResponseError(req, resp, se, context, cVo);
			return;
		}
		...
		try {
			...
			log.error(TangYuanLang.get("web.servlet.invalid.access", requestURI));
			doResponseError(req, resp, se, context, cVo);
			return;
		}
		...
		try {
			...
			doResponseError(traceContext, context, cVo, e, "web.servlet.invalid.convert");
			return;
		}
		...
		try {
			...
			doResponseError(traceContext, context, cVo, e, "web.servlet.invalid.assembly");
			return;
		}
		...
		if (null != validateId) {
			try {
				...
				doResponseError(traceContext, context, cVo, e, "web.servlet.invalid.validate");
				return;
			}
		}
	...
	private void exec(Object traceContext, RequestContext context, ControllerVo cVo) {
		try {
			...
			doResponseError(traceContext, context, cVo, ex, "web.servlet.invalid.exec");
		}
	}
...
//			ServletResponseUtil.responseError(request, response, se);
	...
	private void doResponseSuccess(Object traceContext, RequestContext context, ControllerVo cVo) {
		...
		try {
			...
			log.error(TangYuanLang.get("web.servlet.invalid.onSuccess", context.getPath()), e);
		} finally {
			if (context.isInThread()) {
				WebComponent.getInstance().requestContextThreadLocal.remove();
			}
